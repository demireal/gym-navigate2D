...
class navigate2DEnv(gym.Env):
    ...
    def take_action(self, action, update):
        ...
        if temp_x < 0 or temp_x > (X_STATES - 1) or temp_y < 0 or temp_y > (Y_STATES - 1) or\
            ...
            self.flag = 48 < temp_x < 52 and  43 < temp_y < 47 and 97 < temp_x_tilt < 103 and 94 < temp_rot < 130
            ...
            obs = cv2.resize(self.state, dsize=(self.IN_DIM[1], self.IN_DIM[2]), interpolation=INTERPOLATION)[np.newaxis, :, :]
            reward = 0.1*(1 - self.done)*(1 - self.flag)*(-1 + 2*reinf) + 0.1*(1 - self.done)*(-self.flag) + self.done
            if update == 1:
                self.x_index = temp_x
                self.y_index = temp_y
